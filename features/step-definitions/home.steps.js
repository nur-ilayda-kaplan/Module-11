...
Then('the results should show products related to {string}', async (searchText) => {
...
Then('only products from the selected category should be shown', async () => {
  ...
  assert.isTrue(categoryIsApplied, 'The selected category filter should stay checked');
...
Then('the shown products should match the selected price range', async () => {
  ...
  const pricesMatchSelectedRange = visiblePrices.every(
    (price) => price >= scenarioState.selectedMinPrice && price <= scenarioState.selectedMaxPrice
  );
...
When('the user clicks "More information" on a related product', async () => {
  scenarioState.selectedRelatedProductName = await homePage.openFirstRelatedProduct();
});
