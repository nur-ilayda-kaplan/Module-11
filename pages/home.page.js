...
class HomePage extends BasePage {
  ...
  async isSorted(values, sortValue) {
    ...
    return values.length > 0 && values.every((value, index) => value === expected[index]);
  }
  ...
  async addCurrentProductToCart() {
    ...
    await browser.waitUntil(async () => await this.cartQuantityBadge.isDisplayed(), {
      timeout: 10000,
  ...
  async waitUntilCartLinkIsNotCoveredByOverlay() {
    ...
    await browser.waitUntil(
      ...
      },
      {
        timeout: 15000,
        interval: 200,
        timeoutMsg: 'Cart link stayed covered by an overlay (e.g. success toast)',
      }
    );
  }
  ...
  async openCart() {
    ...
    await browser.waitUntil(async () => (await browser.getUrl()).includes('/checkout'), {
      timeout: 10000,
      timeoutMsg: 'Cart page did not open',
    });
  }
  ...
  async doVisibleCardsContainBrand(brandName) {
    const allCardTexts = await browser.execute(() =>
      Array.from(document.querySelectorAll('a.card')).map((card) => (card.textContent || '').trim())
    );
