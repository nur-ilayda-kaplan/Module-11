...
function pickCapabilities(all) {
  ...
  const picked = all.filter(
    (cap) => cap && typeof cap.browserName === 'string' && cap.browserName.toLowerCase() === want
  );
  ...
  if (want === 'safari' && !isMac) {
    throw new Error('[wdio.conf.js] BROWSER="safari" is only supported on macOS in this project.');
  }
...
exports.config = {
  ...
  cucumberOpts: {
    require: ['./features/support/chai.js', './features/step-definitions/**/*.js'],
    backtrace: false,
    dryRun: false,
    failFast: false,
    snippets: true,
